...
LAYERS = [500, 500, 500, 100, 2]
DENOISING_COST_WEIGHTS = [1.0, 0.3, 0.15, 0.15, 0.15]
L = len(LAYERS) - 1
LEARNING_RATE = 0.02
NOISE_STD = 0.3
...
def model_fn(features, labels, mode, params):
  ...
  logits = tf.layers.dense(outputs, 500)
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    y_preds = tf.nn.sigmoid(forward(logits, is_training=False))
