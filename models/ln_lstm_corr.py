...
LAYERS = [10, 2]
DENOISING_COST_WEIGHTS = [1.0, 0.10]
L = len(LAYERS) - 1
LEARNING_RATE = 0.02
NOISE_STD = 0.9
NUM_EXAMPLES = 60000
...
class LadderNetwork(object):
  ...
  def decoder(self, clean, corr):
    with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
      ...
      return u_cost, corr[L].z, corr[L - 1].z
  ...
  def loss(self, inputs, labels, noise_std=0.0):
    with tf.variable_scope('ladder_network', reuse=tf.AUTO_REUSE):
      ...
      reconstruction_cost, z_est, z_bn = self.decoder(clean, corr)

      loss = classification_cost

      return loss, y_preds, classification_cost, reconstruction_cost, z_est, z_bn
...
def model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    loss, y_preds, classification_cost, reconstruction_cost, z_est, z_bn = ln.loss(logits, labels, params.noise_std)
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    y_preds = ln.predict(logits)
    return tf.estimator.EstimatorSpec(mode, predictions=tf.argmax(y_preds, 1))
